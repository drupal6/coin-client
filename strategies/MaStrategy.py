...
class MaStrategy(object):
    ...
    def on_1hour_kline_data(self, symbol, interval=CandlestickInterval.MIN60):
        ...
        data = self.req_client.get_candlestick(symbol=symbol, interval=interval)
        df = pd.DataFrame(data, columns={"id":0, 'open': 1, 'high': 2, 'low': 3, 'close': 4,
                                         'vol': 5, 'amount': 6, 'count': 7
                                         })

        df = df[['id', 'open', 'high', 'low', 'close', 'vol', 'amount', 'count']]
